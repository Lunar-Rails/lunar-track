...
| T-CB-02 | Elevation | Domain allowlist TS vs SQL | mitigate | Single whitelist; `clovrlabs.com` in both | **closed** |
...
| T-CB-05 | Tampering / Denial | Edge request gate (`src/middleware.ts`) | mitigate | Runnable Next middleware + first-check-in gate | **closed** |
...
| T-CB-02 | **closed** | Fixed: `allowed_domains` table (00033) is the single source of truth. `upsert_profile_on_login` (00034) queries it. `src/lib/auth/allowed-domains.ts` is now a server-only async wrapper. `clovrlabs.com` seeded in 00033. |
...
| T-CB-05 | **closed** | Fixed: `src/proxy.ts` renamed to `src/middleware.ts`, export renamed from `proxy` to `middleware`. First-check-in gate (lines 64–91) is now active. `config.matcher` unchanged. |
...
| 2026-06-04 | 14 | 14 | 0 | manual: T-CB-02 (allowed_domains table, DB-authoritative), T-CB-05 (proxy.ts → middleware.ts) |
...
**Approval:** pending — 0 open threats
