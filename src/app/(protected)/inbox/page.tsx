...
export default async function InboxPage() {
  ...
  if (reportIds.length > 0 && openPeriod) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-body text-lr-muted mt-1">
          {unscoredEmployees.length === 0 ? 'All caught up' : `${unscoredEmployees.length} item${unscoredEmployees.length !== 1 ? 's' : ''} need your attention`}
        </p>
      </div>
      ...
      {!openPeriod && (
        <div className="rounded-[var(--radius-lr-lg)] border border-lr-border bg-lr-glass backdrop-blur-[8px] p-12 text-center">
          ...
          <p className="text-sm text-lr-muted mt-1">No open performance period.</p>
        </div>
      )}
